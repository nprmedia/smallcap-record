"""OpenTimestamps helper: stamp a run's artifacts and upgrade all prior receipts.
    ...
    python scripts/stamp_run.py runs/2026-07-12
...
def run_ots(args: list[str], env: dict) -> int:
...
def main() -> None:
    if len(sys.argv) != 2:
        sys.exit("usage: python scripts/stamp_run.py runs/YYYY-MM-DD")
    ...
    # 1) stamp this run's artifacts that don't have receipts yet
    ...
    # 2) upgrade every prior receipt in the repo (idempotent; leaves .ots.bak
    #    files behind, which are gitignored)
